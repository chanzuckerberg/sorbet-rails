...
Note: You might need to upgrade `sorbet` to the latest version for it to pickup the vendored rbi
automatically.

2. Generate RBI files for routes and models:
```sh
  $ rake rails_rbi:routes
  $ rake rails_rbi:models
```
3. Auto-upgrade the typecheck level of files:
```sh
